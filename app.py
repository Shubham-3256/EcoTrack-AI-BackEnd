# backend/app.py
import os, csv, io, json, logging, subprocess, sys
...
try:
    ...
    logger.error("Firebase Admin SDK failed to initialise: %s", _firebase_err)
...
def firebase_required(f):
    """Verify a Firebase ID token sent as a Bearer token in the Authorization
    header.  On success, attach the resolved DB User to ``g.current_user``.
    Auto-creates a DB row for first-time Firebase users.
    """
    @wraps(f)
    def decorated(*args, **kwargs):
        if not _firebase_initialized:
            logger.error("Firebase not initialised — rejecting request.")
        ...
        try:
            ...
            logger.exception("Firebase token verification error: %s", exc)
            return jsonify({"error": "token_verification_failed"}), 401

        user_email = (decoded_token.get("email") or "").strip().lower()
        user_name  = decoded_token.get("name")

        if not user_email:
            return jsonify({"error": "email_missing_from_token"}), 401

        try:
            user = User.query.filter_by(email=user_email).first()
            if not user:
                # First sign-in via Firebase — provision a DB record
                user = User(
                    email=user_email,
                    name=user_name,
                ...
                db.session.add(user)
                db.session.commit()
                logger.info("Auto-provisioned DB user for %s", user_email)
        except Exception as exc:
            db.session.rollback()
            logger.exception("DB user lookup/create failed: %s", exc)
...
def _maybe_send_alert(user):
    """Fire an email alert if this month's total exceeds the user's threshold."""
...
def export_csv():
    ...
    for r in rows:
        writer.writerow({
            ...
            "company": r.company,
            "kwh":     r.kwh,
            "co2_kg":  round(calculate_emission_from_kwh(r.kwh), 4),
            "notes":   r.notes or "",
        })
...
def export_pdf():
    ...
    story.append(Paragraph(
        f"Total: {total_kwh:.2f} kWh  ·  {total_co2:.2f} kg CO₂", styles["Normal"]
    ))
    story.append(Spacer(1, 12))
